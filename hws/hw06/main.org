...
* Interpreter
  ...
  application and the check if =rator= is a procedure (for instance, it may be
  a primitive procedure). If so, we =apply= the =rator= to the =rands=.
  Otherwise, we the evaluator has encountered a closure. In this case we make a
  recursive call by extending the environment by binding the formal parameters
  of the function (fetched by a call to =get-formals=) to the passed =rands=
  on the body (fetched by a call to =get-body=).
